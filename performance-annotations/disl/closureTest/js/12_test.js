    ...
    YUI().use('moodle-core-notification-confirm', function(Y) {
        ...
        confirmationDialogue.on('complete-yes', function(e) {
            ...
            if (target.test('a')) {
                ...
                Y.log("Element of type " + target.get('tagName') + " is not supported by the M.util.show_confirm_dialog function. Use A, INPUT, or FORM", 'warn', 'javascript-static')
        }, this);
        if (args.cancellabel)
    ...
    var get_htmlelement_size = function(el, prop) {
      ...
      , resize_object = function() {
        ...
        var headerheight = get_htmlelement_size('page-header', 'height')
          , footerheight = get_htmlelement_size('page-footer', 'height')
          , newheight = parseInt(Y.one('body').get('docHeight')) - footerheight - headerheight - 100;
        if (newheight < 400)
            newheight = 400;
        ...
        var colourpicker = {
            ...
            determineColour: function(e) {
                var eventx = Math.floor(e.pageX - e.target.getX())
                  , eventy = Math.floor(e.pageY - e.target.getY())
                  , imagewidth = this.width
                  , imageheight = this.height
                  , factor = this.factor
                  , colour = [255, 0, 0]
                  , matrices = [[0, 1, 0], [-1, 0, 0], [0, 0, 1], [0, -1, 0], [1, 0, 0], [0, 0, -1]]
                  , matrixcount = matrices.length
                  , limit = Math.round(imagewidth / matrixcount)
                  , heightbreak = Math.round(imageheight / 2);
        ...
        M.util.block_hider = M.util.block_hider || (function() {
            ...
            blockhider.prototype = {
                initializer: function(config) {
                    ...
                    var b = this.get('block'), t = b.one('.title'), a = null, hide, show;
...
M.util.add_lightbox = function(Y, node) {
    ...
    var waiticon = Y.Node.create('<img />').setAttrs({
      ...
      , lightbox = Y.Node.create('<div></div>').setStyles({
        opacity: '.75',
        position: 'absolute',
        width: '100%',
        height: '100%',
...
function select_all_in(elTagName, elClass, elId) {
    var inputs = document.getElementsByTagName('input');
    inputs = filterByParent(inputs, function(el) {
        return findParentNode(el, elTagName, elClass, elId)
    });
    for (var i = 0; i < inputs.length; ++i)
        if (inputs[i].type == 'checkbox' || inputs[i].type == 'radio')
            inputs[i].checked = 'checked'
...
function findParentNode(el, elName, elClass, elId) {
    while (el.nodeName.toUpperCase() != 'BODY') {
        if ((!elName || el.nodeName.toUpperCase() == elName) && (!elClass || el.className.indexOf(elClass) != -1) && (!elId || el.id == elId))
            break;
        el = el.parentNode
    }
    ;return el
...
function unmaskPassword(id) {
    var pw = document.getElementById(id)
      , chb = document.getElementById(id + 'unmask');
...
function openpopup(event, args) {
    ...
    if (args.fullscreen) {
        var hackcount = 100
          , get_size_exactly_right = function() {
            ...
            init: function(Y, id, args, nodes) {
                ...
                for (var i in options) {
                    ...
                    if (o.depth == 0) {
                        ...
                        var d = o.depth
                          , current = this.structure[structcount - 1];
          ...
          , embed_function = function() {
            ...
            for (var i = 0; i < M.util.video_players.length; i++) {
                ...
                if (video.width > 0 && video.height > 0) {
                    var src = {
                        src: M.cfg.wwwroot + '/lib/flowplayer/flowplayer-3.2.18.swf.php',
                        width: video.width,
                        height: video.height
                    }
                } else
            ...
            ;for (i = 0; i < M.util.audio_players.length; i++) {
                ...
                ;flowplayer(audio.id, M.cfg.wwwroot + '/lib/flowplayer/flowplayer-3.2.18.swf.php', {
                    plugins: {
                        controls: controls,
                        audio: {
                            url: M.cfg.wwwroot + '/lib/flowplayer/flowplayer.audio-3.2.11.swf.php'
                        }
                    },
                    clip: {
                        url: audio.fileurl,
