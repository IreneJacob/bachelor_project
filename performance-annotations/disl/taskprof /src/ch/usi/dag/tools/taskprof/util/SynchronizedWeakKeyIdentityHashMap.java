...
public final class SynchronizedWeakKeyIdentityHashMap<K, V> {
    ...
    public SynchronizedWeakKeyIdentityHashMap(int initialSize) {
        this.__table = new Entry[initialSize];
        this.__used = 0;
        this.__threshold = (initialSize * 2 ) / 3;
    }

    public SynchronizedWeakKeyIdentityHashMap() {
        this(DEFAULT_INITIAL_SIZE);
    }

    public synchronized void putIfAbsent(final K key, final V value) {
        if (key == null)
    ...
    public synchronized V computeIfAbsent(final K key, final Function<K, V> f) {
        if (key == null)
            throw new IllegalArgumentException();
